...
if history:
    ...
    if st.session_state.get("delete_success_message"):
        st.sidebar.success(st.session_state.pop("delete_success_message"))
    st.sidebar.divider()
    st.sidebar.markdown("### Danger Zone")

    db_to_delete = st.sidebar.selectbox(
        "Select DB to delete",
        options=history,
        format_func=lambda x: f"{x.get('topic', '')} | Sources: {x.get('total_sources', 0)}",
    ...
    confirm_delete = st.sidebar.checkbox(
        "I understand this will permanently delete the selected DB",
        key=widget_key("confirm_delete_db"),
    )

    if st.sidebar.button("Delete Selected Database", key=widget_key("delete_db_btn")):
        if not confirm_delete:
            st.sidebar.warning("Please confirm before deleting.")
        else:
            try:
                deleted_db_name = db_to_delete.get("topic", db_to_delete.get("folder_name", "Database"))
                delete_research_database(db_to_delete["folder_path"])
                ...
                st.session_state["delete_success_message"]=(f"✅ successfully deleted database: {deleted_db_name}")
                st.rerun()
